...
function renderSkeleton() {
    return Array(3).fill(0).map(() => `
        <div class="bg-white rounded-2xl border border-[#E9E1D7] p-4 lg:p-6 animate-pulse">
...
function renderTemplateCard(t, isUserTemplate) {
    return `
        <div onclick="window.handleGridTemplateSelect('${t.id}', ${isUserTemplate})"
            class="group relative bg-white rounded-2xl border border-[#E9E1D7] p-4 lg:p-6 hover:shadow-xl transition-all duration-300 cursor-pointer hover:-translate-y-1">
    ...
    <div class="w-full max-w-6xl mx-auto">
      <div class="flex flex-wrap gap-3 justify-between items-center mb-6">
        <button onclick="window.handleGridBack()"
          class="flex items-center gap-2 text-[#755B00] hover:text-[#4D3D00] transition-colors font-semibold text-base lg:text-lg">
      ...
      <h1 class="font-display text-3xl sm:text-4xl md:text-5xl text-[#1E1B15] mb-3">Elige una Plantilla</h1>
      <p class="text-[#4D4637] text-base sm:text-lg mb-8 sm:mb-10">Selecciona una estructura predefinida para comenzar tu planificación más rápido.</p>

      <div id="user-templates-section" class="mb-10 lg:mb-12 hidden">
        <div class="flex items-center gap-3 mb-4 lg:mb-6">
            <h2 class="font-display text-xl lg:text-2xl text-[#1E1B15]">Mis Plantillas</h2>
            <div class="h-px bg-[#E9E1D7] flex-1"></div>
            <button onclick="window.navigateTo('/my-templates')" class="text-xs lg:text-sm font-medium text-[#755B00] hover:underline whitespace-nowrap">Gestionar Plantillas</button>
        </div>
        <div id="templates-grid-user" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
            ${renderSkeleton()}
        </div>
      </div>
      ...
      <div class="mb-10 lg:mb-12">
        <div class="flex items-center gap-3 mb-4 lg:mb-6">
            <h2 class="font-display text-xl lg:text-2xl text-[#1E1B15]">Plantillas del Sistema</h2>
            <div class="h-px bg-[#E9E1D7] flex-1"></div>
        </div>
        <div id="templates-grid-system" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
            ${renderSkeleton()}
        </div>
      </div>

      <div class="mt-10 lg:mt-12 p-4 lg:p-6 bg-[#FEF3C7] rounded-2xl border border-[#FDE68A] text-center">
        <h3 class="font-display text-lg lg:text-xl text-[#1E1B15] mb-2">¿Ninguna se adapta a ti?</h3>
        <p class="text-[#4D4637] text-sm lg:text-base mb-4">Crea un evento completamente personalizado desde cero.</p>
        <button onclick="window.handleTemplateSelect('custom')"
          class="inline-flex items-center gap-2 px-5 lg:px-6 py-3 bg-[#755B00] text-white rounded-xl font-semibold hover:bg-[#5A4700] transition-colors cursor-pointer text-sm lg:text-base">
          Crear desde cero
          <span class="text-lg lg:text-xl">&rarr;</span>
        </button>
      </div>
    </div>
  `;
