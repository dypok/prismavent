...
### 5.3 Actualizar Evento (Edición Parcial)
* **Endpoint:** `PATCH /events/{event_id}`
* **Seguridad:** Requiere Token (`Authorization: Bearer <token>`).
* **Descripción:** Realiza una edición parcial de los datos de un evento. Únicamente permite actualizar 9 campos editables. Cualquier otro campo es ignorado de forma silenciosa.
* **Campos permitidos:** `name`, `description`, `event_date`, `guest_count`, `max_budget`, `city_id`, `city_custom`, `location`, `visibility_status` (todos opcionales).
* **Reglas de negocio aplicadas:**
  * No se puede modificar un evento que ya esté en estado `"finalizado"` (Retorna `HTTP 400 Bad Request`).
  * El nuevo valor de `event_date` no puede ser en el pasado (Retorna `HTTP 400 Bad Request`).
  * El campo `updated_at` se actualiza de forma automática en la base de datos a `NOW()`.
* **Cuerpo de la Petición (Request Body):**
  ```json
  {
    "name": "Boda de Ana y Luis (Actualizado)",
    "max_budget": 6000.00
  }
  ```
* **Respuesta Exitosa (HTTP 200 OK):**
  * Retorna la misma estructura detallada que `GET /events/{event_id}` (con la lista de ítems, invitados y contadores recalculados).
